...
def main(args):
    ...
    locatedNear=['above', 'behind', 'in front of', 'near', 'under', 'across', 'against', 'along']
    atLocation=['at', 'in', 'on', 'on back of', 'over', 'parked on', 'part of']
    action=['carrying', 'eating', 'flying in', 'holding', 'looking at', 'playing', 'riding', 'sitting on', 'standing on',
    ...
    with open('tmp.tsv', 'w', newline='') as tsvfile:
        ...
        for idx in keep_queries:
            ...
            writer.writerow([CLASSES[probas_sub[idx].argmax()], REL_CLASSES[probas[idx].argmax()], CLASSES[probas_obj[idx].argmax()]])

...
def bbox_check(bbox: List, custom_message, image_width, image_height):
    _check(len(bbox) == 4, f'bbox must be of 4 floats. {len(bbox)} found. {custom_message}')
    left, top, width, height = bbox
    _check(0 <= left < image_width, f'left must be in [0, {image_width}]. {left} found. {custom_message}')
    _check(0 <= top < image_height, f'top must be in [0, {image_height}]. {top} found. {custom_message}')
    _check(0.0 < width <= image_width, f'width must be in (0, {image_width}]. {width} found. {custom_message}')
    _check(0.0 < height <= image_height, f'height must be in (0, {image_height}]. {width} found. {custom_message}')
    _check(left + width <= image_width, f'left + width must be in (0, {image_width}]. {left + width} found. {custom_message}')
    _check(top + height <= image_height, f'top + height must be in (0, {image_height}]. {top + height} found. {custom_message}')
...
def annotation_check(annotation: dict, data_type: str, max_id: int, max_img_id: int, max_category_id: int, image: dict):
    ...
    if data_type == AnnotationKind.OBJECT_DETECTION:
        ...
        bbox_check(annotation['bbox'], ann_msg, width, height)
...
def annotations_check(annotations: List, data_type: str, max_img_id: int, max_category_id: int, quota_limit: QuotaLimit, images: List):
    ...
    for annotation in tqdm(annotations, 'Checking "annotations"...'):
        ...
        annotation_check(annotation, data_type, len(annotations), max_img_id, max_category_id, image)
...
def check_coco_annotation_file(coco_dict: dict, data_type: str, purpose: Purpose):
    ...
    annotations_check(coco_dict['annotations'], data_type, len(coco_dict['images']), len(coco_dict['categories']), quota_limit, coco_dict['images'])
